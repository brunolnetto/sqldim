...
def session():
    ...
    SQLModel.metadata.create_all(engine, tables=[OrderPipeline.__table__])
...
def test_accumulating_update_milestone(session):
    ...
    result = loader.process(
        [{"order_id": "O2", "amount": 50.0, "approved_at": "2024-01-02"}]
    )
    assert result["updated"] == 1

    row = session.exec(
        select(OrderPipeline).where(OrderPipeline.order_id == "O2")
    ).one()
...
def test_accumulating_does_not_overwrite_milestone(session):
    ...
    row = session.exec(
        select(OrderPipeline).where(OrderPipeline.order_id == "O3")
    ).one()
...
def test_accumulating_non_milestone_update(session):
    ...
    assert result["updated"] == 1
    row = session.exec(
        select(OrderPipeline).where(OrderPipeline.order_id == "O4")
    ).one()
