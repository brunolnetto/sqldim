...
class TestBaseSource:
    ...
    def test_event_batch_raises_not_implemented(self):
        ...
        src = _StaticSrc()
        with pytest.raises(NotImplementedError, match="does not model incremental events"):
            src.event_batch()
...
class TestFieldSpecGenerateErrors:
    ...
    def test_post_function_invoked_when_not_none(self):
        ...
        def _post(v, fake, i):
            ...
            return v * 2
        spec = FieldSpec(name="x", kind="randint", sql_type="INTEGER",
                         low=5, high=5, post=_post)
        import faker as _faker
        fake = _faker.Faker()
