...
class TestLazyType4DropViewsExceptionSwallowed:
    ...
    swallow any exception — covers lines 195-196 of _lazy_type4_5.py."""

    def test_drop_view_failure_does_not_propagate(self):
        # Proxy that forwards everything except DROP VIEW (which raises).
        ...
        proxy = _DropViewRaisingProxy(real_con)
        sink = InMemorySink()
        _create_mini_dim(real_con, "dm_mini")
        _create_base_dim(real_con, "dm_base")
        proc = LazyType4Processor(
            natural_key="customer_id",
            base_columns=["name"],
            mini_dim_columns=["age_band", "income_band"],
            base_dim_table="dm_base",
            mini_dim_table="dm_mini",
            mini_dim_fk_col="profile_sk",
            sink=sink,
            con=proxy,
        )
