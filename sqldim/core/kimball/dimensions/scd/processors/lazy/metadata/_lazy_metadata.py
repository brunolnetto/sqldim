...
from sqldim.core.kimball.dimensions.scd.processors.lazy.metadata._lazy_metadata_stream import (
    _MetadataStreamMixin,
)  # noqa: F401
from sqldim.core.kimball.dimensions.scd.processors.lazy.metadata._lazy_metadata_write import (
    _MetadataWriteMixin,
)  # noqa: F401
...
class LazySCDMetadataProcessor(_MetadataWriteMixin, _MetadataStreamMixin):
    ...
    * **Metadata** columns — all remaining source attributes.  Packed into a
    ...
    def process(
        ...
        *now* — ISO-8601 timestamp string used for ``valid_from`` / ``valid_to``.
        ...
        self._register_source(
            source
        )  # TABLE: incoming (parquet → NK + metadata + hash)
        self._register_current_hashes(
            table_name
        )  # TABLE: current_hashes (PG → NK + hash, once)
    ...
    def _register_current_hashes(self, table_name: str) -> None:
        ...
        2. **Spill-to-disk eligibility** — once the rows live in a DuckDB
           ...
           this safety valve (see :func:`sqldim.core.processors.lazy_scd_loader.run_lazy_scd`).
        """
        nk_select = ", ".join(self._nk_cols)
