    ...
    for c in cols:
        if native[c].dtype == "object":
            native[c] = native[c].apply(
                lambda x: _json.dumps(x, sort_keys=True) if isinstance(x, (dict, list)) else x
            )
    return native
...
def _hash_polars_frame(native: Any) -> Any:
    ...
    checksums = [
        hashlib.md5(
            (_json.dumps(v, sort_keys=True) if isinstance(v, (dict, list)) else str(v)).encode("utf-8")
        ).hexdigest()
...
class NarwhalsSCDProcessor:
    ...
    def _open_rows(self, current_scd: nw.DataFrame) -> nw.DataFrame:
        if "is_current" in current_scd.columns:
            return current_scd.filter(nw.col("is_current") == True)
        return current_scd
    ...
    def _build_closed_frame(
        ...
        if first_col:
            return current_open.filter(nw.col(first_col).is_null() & ~nw.col(first_col).is_null())
