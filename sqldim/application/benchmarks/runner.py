...
Usage::
    ...
    python -m sqldim.application.benchmarks.runner                          # all groups, safe tiers
    ...
    python -m sqldim.application.benchmarks.runner A A.cnpj B               # mixed selection
    python -m sqldim.application.benchmarks.runner --max-tier m             # cap at 1M rows
    python -m sqldim.application.benchmarks.runner --report json            # output format
    python -m sqldim.application.benchmarks.runner --fail-on-regression     # exit 1 on regressions

Case selection
--------------
Each positional argument is either a *group ID* (e.g. ``A``) or a
*group ID + element filter* (e.g. ``A.products``).  When an element filter
is given, only cases whose ``case_id`` contains the element substring are
run; all others are skipped before any data is generated.

Examples::

    A              → all cases in group A
    A.products     → only cases in A whose case_id contains "products"
    B.s            → only tier-s cases in group B
    A.products B   → filtered A + all of B
...
def _parse_cases(
    ...
    """Parse positional CASE tokens into ``{group_id: elements | None}``.

    ``None`` means *all elements* in that group.  A list means only cases
    whose ``case_id`` contains at least one of the listed substrings.

    Rules
    -----
    * ``"A"``            → ``{"A": None}``         (run all cases)
    * ``"A.products"``   → ``{"A": ["products"]}``
    * ``"A.products" + "A"``  → ``{"A": None}``  (group-level wins)
    * ``"A.products" + "A.employees"`` → ``{"A": ["products", "employees"]}``
    """
    ...
    for token in tokens:
        if "." in token:
            ...
            group_id, element = token.upper(), None

        if group_id not in GROUP_MAP:
            valid = ", ".join(sorted(GROUP_MAP))
            raise ValueError(
                f"Unknown group {group_id!r} in {token!r}. "
                f"Valid group IDs: {valid}"
            )
...
def _build_groups_epilog() -> str:
    """Return a formatted table of group IDs and descriptions for --help."""
    lines = [
        "available groups:",
        "",
    ]
    for gid, desc in GROUP_DESCRIPTIONS.items():
        lines.append(f"  {gid}  {desc}")
    lines += [
        "",
        "case selection examples:",
        "  A                run all cases in group A",
        "  A.products       only cases whose case_id contains 'products'",
        "  A B.employees    combined: all of A plus filtered B",
        "  C --max-tier m   group C capped at 1 M-row tier",
        "",
    ]
    return "\n".join(lines)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        metavar="CASE",
        help=(
            "Groups or group.element pairs to run.  Omit to run all groups.  "
            "Examples: 'A' (all of group A), 'A.products' (cases in A whose "
            "case_id contains 'products'), 'A B.employees' (all A + filtered B)."
        ),
    )
    p.add_argument(
...
def main(argv=None) -> int:
    ...
    # Ordered list of (group_id, element_filter | None) to run
    if case_selection is None:
        groups_to_run: list[tuple[str, list[str] | None]] = [
            (g, None) for g in GROUP_MAP
        ]
    else:
        groups_to_run = list(case_selection.items())
    ...
    if case_selection is not None:
        ...
        for gid, ef in groups_to_run:
            label = f"{gid}.{{{','.join(ef)}}}" if ef else gid
            lines.append(label)
    ...
    with tempfile.TemporaryDirectory(prefix="sqldim_bench_") as tmp_dir:
        with BenchmarkDatasetGenerator(tmp_root=tmp_dir) as gen:
            ...
            for group_id, element_filter in groups_to_run:
                ...
                filter_label = (
                    f" [filter: {element_filter}]" if element_filter else ""
                )
                print(_bold(f"  ▶  Group {group_id}: {desc}{filter_label}"))
                ...
                if element_filter is not None:
                    ...
                    results = [
                        r for r in results
                        if any(e in r.case_id for e in element_filter)
                    ]
                    skipped = pre_count - len(results)
                    if skipped:
                        print(_dim(
                            f"     Filtered {skipped} case(s) not matching "
                            f"{element_filter!r}"
                        ))
                    if not results:
                        print(_dim(
                            f"     No cases in group {group_id} matched "
                            f"filter {element_filter!r} — skipping."
                        ))
                        continue
