...
def group_a_scan_regression(
    ...
    for profile in ["products", "employees", "saas_users"]:
        ...
        results.append(
            _run_scd2_batch(
                ds, f"A_{profile}_scan_regression", "A_scan_regression", "xs", temp_dir
            )
        )
        ds.cleanup()
    ...
    results.append(
        _run_metadata_batch(
            ds, "A_cnpj_metadata_scan_regression", "A_scan_regression", "xs", temp_dir
        )
    )
    ds.cleanup()
    return results
...
def group_b_memory_safety(
    ...
    for profile, tier, proc_type in cases:
        ...
        r = (
            _run_metadata_batch(ds, cid, "B_memory_safety", tier, temp_dir)
            if proc_type == "meta"
...
def group_c_throughput_scaling(
    ...
    for tier in tier_order[: max_idx + 1]:
        try:
            ...
            results.append(
                BenchmarkResult(
                    case_id=f"C_products_{tier}_throughput",
                    group="C_throughput",
                    profile="products",
                    tier=tier,
        ...
        results.append(
            _run_scd2_batch(
                ds, f"C_products_{tier}_throughput", "C_throughput", tier, temp_dir
            )
        )
        ds.cleanup()
    return results
