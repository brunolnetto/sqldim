...
from sqldim.application.examples.real_world.supply_chain.models import (
    Supplier,
    Warehouse,
    SKU,
    ShipmentEdge,
...
async def _sc_pillar1_scd_warehouses(engine) -> None:
    ...
    src = supply_chain_dataset["warehouses"]
...
async def _sc_pillar2_shipment_graph(engine) -> tuple:
    ...
    sku_src = supply_chain_dataset["skus"]
    with Session(engine) as session:
...
EXAMPLE_METADATA = {
    ...
    "description": "SCD Type 2 + shipment graph + cumulative stock arrays + schema graph + DGM analytics",
    "entry_point": "run_supply_chain_showcase",
}
