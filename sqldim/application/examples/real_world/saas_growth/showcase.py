...
---------------------------------------
Vectorised SCD processing (Narwhals) + referral graph + bitmask retention.
"""
...
EXAMPLE_METADATA = {
    "name": "saas",
    "title": "SaaS Growth",
    "description": "Vectorised SCD (Narwhals) + referral graph + bitmask retention",
    "entry_point": "run_saas_showcase",
}
...
async def run_saas_showcase():
    ...
    NarwhalsSCDProcessor(natural_key=["email"], track_columns=["plan_tier", "meta"])
    ...
    print(
        f"   User '{alice_v.email}' found {len(referrals)} referral(s) via Recursive CTE."
    )
    ...
    print(
        f"   User '{u2.email}' L7 Engagement Score: {bob_activity.l7(ref_date)} / 7 days"
    )
    print("   Stored 5 active days in a single integer-encoded field.\n")
