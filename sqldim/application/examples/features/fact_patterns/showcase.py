...
Run:
    PYTHONPATH=. python -m sqldim.application.examples.features.fact_patterns.showcase
"""
...
def _order_summary(r: tuple) -> str:
    vals = r[1:5]
    filled = " → ".join(
        filter(None, (_stage_label(s, v) for s, v in zip(_STAGES, vals)))
    )
...
def example_05_ecommerce_orders() -> None:
    ...
    with DuckDBSink(path) as sink:
        loader = LazyAccumulatingLoader(
            table="fact_order_pipeline",
            match_column="order_id",
            milestone_columns=["paid_at", "shipped_at", "delivered_at"],
            sink=sink,
            con=sink._con,
        )
        r = loader.process(src.placed_events())
        print(f"  T1 placed    → {r}")
        r = loader.process(src.paid_events())
        print(f"  T2 paid      → {r}")
        r = loader.process(src.shipped_events())
        print(f"  T3 shipped   → {r}")
        r = loader.process(src.delivered_events())
        print(f"  T4 delivered → {r}")
...
EXAMPLE_METADATA = {
    "name": "fact-patterns",
    "title": "Fact Patterns",
    "description": "Examples 5-8: transaction, periodic, accumulating and bitmask facts",
