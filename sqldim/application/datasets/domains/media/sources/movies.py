"""MoviesSource — media domain source."""
...
class MoviesSource(BaseSource):
    ...
    def __init__(self, n_actors: int = 8, n_movies: int = 5, seed: int = 42) -> None:
        ...
        self._actors: list[dict[str, Any]] = [
            {"id": i + 1, "name": fake.name()} for i in range(n_actors)
        ]
    ...
    def setup(
        ...
        rows = ", ".join(f"({a['id']}, '{_esc(a['name'])}')" for a in self._actors)
