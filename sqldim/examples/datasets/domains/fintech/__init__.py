...
=============================================
Faker-backed OLTP simulator for the fintech real-world example.

Provides synthetic account, counterparty, and transaction data matching the
``Account``, ``Counterparty``, and ``TxnFact`` schemas in ``models.py``.
Feeds the SCD2 + graph traversal + array-metric pipeline in ``showcase.py``.

Sources
-------
``AccountsSource``      — bank accounts with risk-tier upgrades / downgrades.
``CounterpartiesSource``— external institutions (name/sanction updates).
``TransactionsSource``  — raw payment events (debit/credit ledger entries).
"""
...
@DatasetFactory.register("fintech_counterparties")
class CounterpartiesSource(SchematicSource):
    """Synthetic counterparty OLTP source keyed on ``bic_code``."""

    spec = _COUNTERPARTIES_SPEC
    schema_name = "source"

    def __init__(self, n_entities: int = 25, *, seed: int = 42) -> None:
        super().__init__(
            spec=_COUNTERPARTIES_SPEC,
            schema_name="source",
...
@DatasetFactory.register("fintech_transactions")
class TransactionsSource(SchematicSource):
    """Synthetic payment ledger OLTP source."""

    spec = _TRANSACTIONS_SPEC
    schema_name = "source"

    def __init__(self, n_entities: int = 200, *, seed: int = 42) -> None:
        super().__init__(
            spec=_TRANSACTIONS_SPEC,
            schema_name="source",
            n_entities=n_entities,
