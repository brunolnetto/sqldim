...
_COLLEGES = [
    "Duke", "Kentucky", "UNC", "Kansas", "Michigan", "UCLA",
    "Indiana", "Gonzaga", "Syracuse", "Arizona", "Louisville",
]
_COUNTRIES = ["USA", "USA", "USA", "France", "Nigeria", "Australia",
              "Canada", "Spain", "Serbia", "Germany"]
    ...
    schemas={
        "source": EntitySchema(
            ...
            fields=[
                ...
                FieldSpec("draft_year",  "VARCHAR", kind="randint", low=1995, high=2020,
                          post=lambda v, f, i: str(v)),
                FieldSpec("draft_round", "VARCHAR", kind="choices",
                          choices=["1", "1", "1", "2"]),
                FieldSpec("draft_number","VARCHAR", kind="randint", low=1, high=60,
                          post=lambda v, f, i: str(v)),
                FieldSpec("gp",          "DOUBLE",  kind="randint",  low=20, high=82),
                FieldSpec("pts",         "DOUBLE",  kind="uniform",  low=2.0, high=32.0, precision=1),
                FieldSpec("reb",         "DOUBLE",  kind="uniform",  low=1.0, high=14.0, precision=1),
                FieldSpec("ast",         "DOUBLE",  kind="uniform",  low=0.5, high=11.0, precision=1),
                FieldSpec("netrtg",      "DOUBLE",  kind="uniform",  low=-15.0, high=20.0, precision=1),
                FieldSpec("oreb_pct",    "DOUBLE",  kind="uniform",  low=0.02, high=0.22, precision=3),
                FieldSpec("dreb_pct",    "DOUBLE",  kind="uniform",  low=0.05, high=0.35, precision=3),
                FieldSpec("usg_pct",     "DOUBLE",  kind="uniform",  low=0.10, high=0.38, precision=3),
                FieldSpec("ts_pct",      "DOUBLE",  kind="uniform",  low=0.40, high=0.75, precision=3),
                FieldSpec("ast_pct",     "DOUBLE",  kind="uniform",  low=0.05, high=0.55, precision=3),
            ],
        ),
    },
    events=EventSpec(
        changes=[
            ...
            ChangeRule(
                ...
                mutate=lambda old, row, fake: round(old * random.uniform(0.85, 1.15), 1),
            ),
            ChangeRule(
                field="reb",
                condition=lambda i, row: random.random() < 0.8,
            ...
            ChangeRule(
                ...
                mutate=lambda old, row, fake: min(82, max(20, int(old) + random.randint(-10, 10))),
            ),
        ],
