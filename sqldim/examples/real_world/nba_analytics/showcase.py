...
def _print_scd_versions(scd_versions: list) -> None:
    ...
    for v in scd_versions:
        ...
        print(
            f"     - {v.valid_from} to {v.valid_to or 'Present'}: {v.scoring_class} ({status})"
        )
...
async def run_showcase():
    ...
    j1993 = Player(
        ...
        seasons=[
            SeasonStats(
                season=1991, pts=31.5, ast=5.5, reb=6.0, weight=198
            ).model_dump(),
            SeasonStats(
                season=1992, pts=30.1, ast=6.1, reb=6.4, weight=198
            ).model_dump(),
            SeasonStats(
                season=1993, pts=32.6, ast=5.5, reb=6.7, weight=198
            ).model_dump(),
        ],
    )
    ...
    j1998 = Player(
        ...
        + [
            SeasonStats(
                season=1996, pts=30.4, ast=4.3, reb=6.6, weight=215
            ).model_dump(),
            SeasonStats(
                season=1997, pts=29.6, ast=4.3, reb=5.9, weight=215
            ).model_dump(),
            SeasonStats(
                season=1998, pts=28.7, ast=3.5, reb=5.8, weight=215
            ).model_dump(),
        ],
    )
    ...
    session.commit()
    print("✅ Pillar 1: Cumulative snapshots built for Michael Jordan")
    print("   Stored dense history in 'player' table (1993, 1995, 1998).\n")
    ...
    print("✅ Pillar 2: SCD Type 2 timeline generated via backfill_scd2()")
    ...
    _print_scd_versions(scd_versions)
    print()
    ...
    historical_state = session.exec(
        ...
        .where(PlayerSCD.valid_to.is_(None) | (PlayerSCD.valid_to > target_year))
    ).first()

    print("✅ Pillar 3: Semantic 'AS OF' Query (Time Travel)")
    if historical_state:  # pragma: no cover
        status = (
            "Active" if historical_state.is_active else "Retired"
        )  # pragma: no cover
        print(
            f"   Status in {target_year}: {status} (Scoring: {historical_state.scoring_class})\n"
        )  # pragma: no cover
    ...
    edge = PlaysInEdge(
        subject_id=j1998.id, object_id=g1.id, pts=45.0, start_position="G"
    )
    session.add(edge)
    session.commit()
    ...
    print("✅ Pillar 4: Graph Projection (Dual-Paradigm)")
    print(f"   Vertex '{jordan_v.player_name}' played Game ID: {neighbors[0].game_id}")
    print("   Path found via Recursive CTE projection.\n")
