...
from sqldim import FactModel, SCD2Mixin, VertexModel, EdgeModel, DatelistMixin
...
class User(VertexModel, SCD2Mixin, table=True):
    ...
    meta: Dict[str, Any] = Field(
        default_factory=dict, sa_column=Column(JSON, info={"scd": 2})
    )
...
class UserActivity(FactModel, DatelistMixin, table=True):
    ...
    dates_active: List[date] = Field(default_factory=list, sa_column=Column(JSON))
